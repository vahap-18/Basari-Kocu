...
function generateAdvice(profile: PersonalityProfile | null) {
  if (!profile)
    return ["Kişilik testini tamamla, sana özel tavsiyeler burada görünecek."];
  const adv: string[] = [];
  ...
  if (focus >= 4)
    adv.push(
      "Uzun odak seanslarında başarılısın — 50 dk çalışma + 10 dk mola deneyebilirsin.",
    );
  else if (focus === 3)
    adv.push("Orta seviye odak; 25 dk çalışma + 5 dk mola iyi gelir.");
  else
    adv.push(
      "Kısa ve sık seanslar (20 dk) verimli olabilir. Dikkat dağılması için çevrenizi düzenleyin.",
    );

  if (procrastinate <= 2)
    adv.push("Erteleme eğilimlerin düşük; hedeflerini büyütebilirsin.");
  else if (procrastinate <= 4)
    adv.push(
      "Bazen erteleme oluyor; Pomodoro başlangıcında 1 küçük ödül belirle.",
    );
  else
    adv.push(
      "Erteleme yüksek; günlük küçük görevlerle (2–3) başla ve başarı hissini kullan.",
    );

  if (resilience >= 4)
    adv.push(
      "Zorluklara karşı dayanıklısın; zor konuları sabah saatlerine koy.",
    );
  if (curiosity >= 4)
    adv.push("Merakın güçlü; konuları sorgulayarak not al ve bağlantılar kur.");
...
export const AICoach: React.FC = () => {
  ...
  const [messages, setMessages] = useState<
    { from: "coach" | "user"; text: string }[]
  >([]);
  const [input, setInput] = useState("");
  const [autoTips, setAutoTips] = useState(
    () => localStorage.getItem("coach-auto") !== "0",
  );
  ...
  function handleSend() {
    ...
    if (lower.includes("program") || lower.includes("plan")) {
      ...
      reply =
        "Kendine küçük ödüller koy, sabit bir başlangıç rutini (5 dk) uygula.";
    } else if (lower.includes("focus") || lower.includes("dikkat")) {
      reply =
        "Dış uyarıcıları azalt, telefonunu başka bir odada bırak veya bildirimleri sessize al.";
    }
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Kişilik testine göre sana özel öneriler sunar.
            </p>
          </div>
          <div>
            <label className="inline-flex items-center gap-2 text-sm">
              <input
                type="checkbox"
                checked={autoTips}
                onChange={(e) => setAutoTips(e.target.checked)}
              />
              Günlük ipuçları
            </label>
          </div>
        ...
        <div className="space-y-2">
          {messages.map((m, i) => (
            <div
              ...
              className={
                m.from === "coach"
                  ? "text-sm bg-secondary/10 p-2 rounded-lg"
                  : "text-sm text-right p-2"
              }
            >
              <div>{m.text}</div>
            </div>
