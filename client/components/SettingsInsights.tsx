...
export default function SettingsInsights({ profile }: { profile: any }) {
  ...
  React.useEffect(() => {
    ...
    const onStorage = (ev: StorageEvent) => {
      if (ev.key === "personality-profile") {
        try {
          setLocalProfile(ev.newValue ? JSON.parse(ev.newValue) : null);
        } catch {}
      }
    };
  ...
  const getScore = (key: string, fallbackKeys: string[] = []) => {
    if (typeof scores[key] === "number")
      return Math.max(0, Math.min(5, scores[key]));
    for (const k of fallbackKeys) {
      if (typeof scores[k] === "number")
        return Math.max(0, Math.min(5, scores[k]));
    }
    return 0;
  };

  const entrepreneurship = getScore("entrepreneurship", [
    "leadership",
    "curiosity",
  ]);
  ...
  const progress = allVals.length
    ? Math.round(
        (allVals.reduce((s, v) => s + v, 0) / (allVals.length * 5)) * 100,
      )
    : 0;
  ...
  React.useEffect(() => {
    setAnimE(toPercent(entrepreneurship));
  }, [entrepreneurship]);
  React.useEffect(() => {
    setAnimW(toPercent(work));
  }, [work]);
  React.useEffect(() => {
    setAnimC(toPercent(courage));
  }, [courage]);
  React.useEffect(() => {
    setAnimP(progress);
  }, [progress]);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div
          className="p-3 rounded-2xl border bg-card"
          ...
          <div className="flex items-center justify-between mb-2">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                Yaratıcılık, liderlik ve fırsatları görme yeteneği
              </div>
            </div>
        ...
        <div
          className="p-3 rounded-2xl border bg-card"
          style={{ animation: "breathe 6.5s ease-in-out infinite" }}
        >
          <div className="flex items-center justify-between mb-2">
            <div>
              <h4 className="font-semibold">Çalışma Düzeni</h4>
              <div className="text-sm text-muted-foreground">
                Planlama, disiplini ve görev tamamlama
              </div>
            </div>
            <div className="text-right">
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                Zorluklarla başa çıkma ve risk alma eğilimi
              </div>
            </div>
            <div className="text-right">
        ...
        <div
          className="p-3 rounded-2xl border bg-card"
          style={{ animation: "breathe 6.8s ease-in-out infinite" }}
        >
          <div className="flex items-center justify-between mb-2">
            <div>
              <h4 className="font-semibold">Genel İlerleme</h4>
              <div className="text-sm text-muted-foreground">
                Tüm puanların ortalaması
              </div>
            </div>
            <div className="text-right">
          ...
          <div className="mt-2">
            ...
            <div className="mt-2 text-sm text-muted-foreground">
              Örnek aksiyonlar: Günlük hedef belirle, haftalık retrospektif yap,
              riskleri küçük adımlara böl.
            </div>
          </div>
        </div>
      </div>
